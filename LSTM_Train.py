...
#--------------  debug  --------------------------#
# output1 = open(os.path.join(os.getcwd(), 'output1.debug'), 'w')
...
input_data = tf.compat.v1.placeholder(tf.float32, [None, sequence_length, stft_size])
clean_data = tf.compat.v1.placeholder(tf.float32, [None, sequence_length, stft_size])
sequence_length_tensor = tf.compat.v1.placeholder(tf.int32, (None))
...
#clean_flies_vec = map(zip_longest, clean_files_vec)
